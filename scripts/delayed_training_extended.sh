...
for DELAY in "${DELAYS[@]}"; do
    for RANDOM_POLICY_SEED in "${SEEDS[@]}"; do
        for TASK in "${TASKS[@]}"; do
            srun python dreamerv3/train.py --logdir ./logdir/train/delayed/single_step/extended_mlp/$(echo ${OBS_TYPE} | sed 's/.*_//')/${TASK}/d=${DELAY}/$(date "+%Y%m%d-%H%M%S")\
