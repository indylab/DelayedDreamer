...
class JAXAgent(embodied.Agent):
  ...
  def policy_delayed(self, obs, action_history, state=None, 
                     mode='train',
                     delayed_actor='normal',
                     imagination_mode='latent'):
    obs = obs.copy()
    action_history = action_history.copy()
    obs = self._convert_inps(obs, self.policy_devices)
    action_history = self._convert_inps(action_history, self.policy_devices)
    ...
    if delayed_actor == 'normal' and imagination_mode == 'latent':
      (outs, state), _ = self._policy_delayed_latent(varibs, rng, obs, action_history, state, mode=mode)
    elif delayed_actor == 'normal' and imagination_mode == 'recon':
      (outs, state), _ = self._policy_delayed_recon(varibs, rng, obs, action_history, state, mode=mode)
    elif delayed_actor in ['memoryless', 'extended_state_mlp']:
      (outs, state), _ = self._policy_delayed_extended(varibs, rng, obs, action_history, state, mode=mode)
    else:
  ...
  def _transform(self):
    ...
    self._policy_delayed_latent = nj.pure(self.agent.policy_delayed_latent)
    self._policy_delayed_recon = nj.pure(self.agent.policy_delayed_recon)
    self._policy_delayed_extended = nj.pure(self.agent.policy_delayed_extended)
    ...
    if len(self.policy_devices) == 1:
      ...
      self._policy_delayed_latent = nj.jit(self._policy_delayed_latent, static=['mode'], **kw)
      self._policy_delayed_recon = nj.jit(self._policy_delayed_recon, static=['mode'], **kw)
      self._policy_delayed_extended = nj.jit(self._policy_delayed_extended, static=['mode'], **kw)
    else:
      ...
      self._policy_delayed_latent = nj.pmap(self._policy_delayed_latent, 'i', static=['mode'], **kw)
      self._policy_delayed_recon = nj.pmap(self._policy_delayed_recon, 'i', static=['mode'], **kw)
      self._policy_delayed_extended = nj.pmap(self._policy_delayed_extended, 'i', static=['mode'], **kw)
  ...
  def _dummy_batch(self, obs_space, act_space, batch_dims):
    spaces = {**obs_space, **act_space}
    spaces = list(spaces.items())
    data = {k: np.zeros(v.shape, v.dtype) for k, v in spaces}
    if self.delay_config.delayed_training:
      for k, v in act_space.items():
        data[k + '_next'] = np.zeros((self.delay_config.maximum_delay,) + v.shape, v.dtype)
      data['delay'] = np.zeros((1, ), np.int32)
