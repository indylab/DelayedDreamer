...
def main(argv=None):
  ...
  logdir_new = config.logdir + f"_{config.task}"
  args = embodied.Config(
      **config.run, logdir=logdir_new, 
      batch_steps=config.batch_size * config.batch_length,
      delayed_actor=config.delay.delayed_actor,
      imagination_mode=config.imagination_mode,)
  ...
  try:
    ...
    if args.script == 'train':
    ...
    elif args.script == 'train_delayedeval':
      replay = make_replay(config, logdir / 'replay')
      eval_replay = make_replay(config, logdir / 'eval_replay', is_eval=True)
      env = make_envs(config)
      eval_env = make_envs(config)  # mode='eval'
      cleanup += [env, eval_env]
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.train_delayedeval(
          agent, env, eval_env, replay, eval_replay, logger, args)

    elif args.script == 'delayedtrain_delayedeval':
      replay = make_replay(config, logdir / 'replay')
      eval_replay = make_replay(config, logdir / 'eval_replay', is_eval=True)
      env = make_envs(config)
      eval_env = make_envs(config)  # mode='eval'
      cleanup += [env, eval_env]
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.delayedtrain_delayedeval(
          agent, env, eval_env, replay, eval_replay, logger, args)
    ...
    elif args.script == 'eval_only_delay':
        env = make_envs(config)  # mode='eval'
        cleanup.append(env)
        agent = agt.Agent(env.obs_space, env.act_space, step, config)
        embodied.run.eval_only_delay(agent, env, logger, args)

    elif args.script == 'random_eval':
        env = make_envs(config)  # mode='eval'
        cleanup.append(env)
        agent = agt.Agent(env.obs_space, env.act_space, step, config)
        embodied.run.random_eval(agent, env, logger, args)
...
def make_logger(parsed, logdir, step, config, postfix: str = ''):
  multiplier = config.env.get(config.task.split('_')[0], {}).get('repeat', 1)
  logger = embodied.Logger(step, [
      embodied.logger.TerminalOutput(config.filter),
      embodied.logger.JSONLOutput(logdir, f'metrics{postfix}.jsonl'),
      embodied.logger.JSONLOutput(logdir, f'scores{postfix}.jsonl', 'episode/score'),
...
def make_envs(config: embodied.Config, **overrides):
  envs = []
...
def make_env(config, **overrides):
  ...
  ctor = {
      ...
      'mujoco': 'embodied.envs.from_gym:FromGym',
  }[suite]
...
def wrap_env(env, config):
  ...
  if config.delay.delayed:
    assert config.delay.delay_length >= 0, \
      ...
      f"Currently, config.delay.delay_length is {config.delay.delay_length}."
    assert config.delay.maximum_delay >= config.delay.delay_length, \
      "Something wrong. config.delay.maximum_delay must be greater than or equal to config.delay.delay_length. " + \
      f"Currently, config.delay.maximum_delay is {config.delay.maximum_delay} and config.delay.delay_length is {config.delay.delay_length}."
    env = wrappers.DelayedEnv(env=env, 
                              fixed_delay=config.delay.delay_length,
                              maximum_delay=config.delay.maximum_delay,)
  return env
