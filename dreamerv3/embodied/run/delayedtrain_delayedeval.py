...
def delayedtrain_delayedeval(
    agent, 
    train_env, 
    eval_env, 
    train_replay, 
    eval_replay, 
    logger: embodied.core.logger.Logger, 
    args):
  ...
  updates: embodied.Counter = embodied.Counter()
  metrics: embodied.Metrics = embodied.Metrics()
  ...
  timer.wrap('env', train_env, ['step', 'act'])
  ...
  def per_episode(ep, mode):
    ...
    }, prefix=('episode' if mode == 'train' else f'{mode}_{prefix_delay}episode'))
    print(f'Episode has {length} steps and return {score:.1f}.')  # TODO
    stats = {}
  ...
  policy_train = lambda *params: agent.policy_delayed(*params, mode='explore' if should_expl(step) else 'train',
  ...
  while len(train_replay) < max(args.batch_steps, args.train_fill):
    driver_train.delayed_call(policy_train, random_agent.policy, steps=100, prefill=True)
  print('Prefill eval dataset.')
  while len(eval_replay) < max(args.batch_steps, args.eval_fill):
    driver_eval.delayed_call(policy_eval, random_agent.policy, steps=100, prefill=True)
  logger.add(metrics.result())
  logger.write()


  if args.random_policy_seed != -1:
    random_agent.set_seed(args.random_policy_seed)
  ...
  while step < args.steps:
    if should_eval(step):
      ...
      driver_eval.reset()
      driver_eval.delayed_call(policy_eval, random_agent.policy, episodes=max(len(eval_env), args.eval_eps))
    driver_train.delayed_call(policy_train, random_agent.policy, steps=100)
    if should_save(step):
      checkpoint.save()
  logger.write()
  logger.write()
