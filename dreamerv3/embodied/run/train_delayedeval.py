...
def train_delayedeval(
  ...
  driver_eval.on_step(eval_replay.add)  # TODO: what does this do? TODO: necessary?
  ...
  dataset_eval = agent.dataset(eval_replay.dataset)  # TODO: is it used?
  state = [None]  # To be writable from train step function below.
  batch = [None]
  def train_step(tran, worker):
    for _ in range(should_train(step)):
      with timer.scope('dataset_train'):
        batch[0] = next(dataset_train)  # TODO: why batch[0]?
  ...
  assert args.delay_eval_version in ["latent", "recon"], f"{args.delay_eval_version} is not in ['latent', 'recon']."
  kwargs = {"delayed_policy_mode": args.delay_eval_version}
  policy_eval = lambda *args: agent.policy_delayed(*args, **kwargs, mode='eval')
