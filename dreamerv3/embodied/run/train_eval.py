...
def train_eval(
  ...
  if args.save_every == -1:  # save every
    should_save = embodied.when.Every(args.eval_every, args.eval_initial)
  else:
  ...
  while step < args.steps:
    ...
    if should_save(step):
      if args.save_every == -1:
        checkpoint.save(logdir / f'checkpoint_step{int(step):0>{7}}.ckpt')
      else:
        checkpoint.save()
  logger.write()
  logger.write()
