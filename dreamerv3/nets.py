...
class RSSM(nj.Module):
  ...
  def multistep(self, embed, action, is_first, state=None, d=None, max_d=None):
    # d -> if is not None: compute only priors of distance d, for d=0 --> immediate prior
    # max_d --> if is not None: compute all priors up to d, otherwise set it to batch_length
    # output: post and prior --> (batch_size, batch_length, max_d,...) if d is None else (batch_size, batch_length,...)
    #                             (i, j, k, ...) --> batch_i: start with posterior at time j and imagine for k steps
    max_d = max_d if max_d else action.shape[1]

    batch_size = action.shape[0]
    batch_length = action.shape[1]
    repeat = lambda x: x[:, :, jnp.newaxis, ...].repeat(batch_length, axis=2)
    repeat_axis1 = lambda x: x[:, jnp.newaxis, ...].repeat(batch_length, axis=1)
    swap = lambda x: x.transpose([1, 0] + list(range(2, len(x.shape))))
    shape = embed.shape[:2]
    merge = lambda x: x.reshape((-1, ) + x.shape[2:])
    unmerge = lambda x: x.reshape(shape + x.shape[1:])

    state = self.initial(action.shape[0]) if state is None else state
    post, _ = self.observe(embed, action, is_first, state)

    input_state = {k: jnp.concatenate([state[k][:, jnp.newaxis, ...], post[k][:, :-1]], axis=1) for k in state.keys()}

    _mask = lambda x, y: jnp.einsum('ij...,ij->ij...', x, y.astype(x.dtype))
    is_first = cast(is_first)
    input_state, action = jax.tree_util.tree_map(
      lambda x: _mask(x, (1.0-is_first)), (input_state, action))
    input_state = jax.tree_util.tree_map(
      lambda x, y: x + _mask(y, is_first),
      input_state, {k: v.reshape((is_first.shape + v.shape[1:])) for k, v in self.initial(is_first.size).items()})
      
    input_state = {k: merge(v) for k, v in input_state.items()}
    # input_state.items -> (b*batch_length, ...) and input_state[i*batch_length:i*(batch_length+1)] = state[i, :], post[i, :-1]
    
    input_actions = repeat_axis1(action)
    input_actions = merge(input_actions)
    input_actions = swap(input_actions)
    def indep_roll(arr, shifts, axis=1):
        arr = jnp.swapaxes(arr,axis,-1)
        all_idcs = jnp.ogrid[[slice(0,n) for n in arr.shape]]
        all_idcs[-1] = all_idcs[-1] - shifts[:, jnp.newaxis]
        result = arr[tuple(all_idcs)]
        arr = jnp.swapaxes(result,-1,axis)
        return arr
    rolls = jnp.tile(jnp.arange(batch_length, 0, -1), batch_size)
    input_actions = indep_roll(input_actions, rolls, axis=0)
    # input_actions -> (batch_length, b * batch_length, ...) and input_actions[i, j*batch_length:j*(batch_length+1)] = action[j, j:] + action[j, :j]

    prior = jaxutils.scan(self.img_step, input_actions, input_state, self._unroll)
    prior = {k: swap(v) for k, v in prior.items()}
    prior = {k: unmerge(v) for k, v in prior.items()}
    # prior items shape (batch, batch_length, batch_length, ...)
    # prior items [b, i, j, ...] --> batch b, start with posterior at time i and imagine for j steps

    post = {k: repeat(v) for k, v in post.items()}
    # post.items.size --> (batch_size, batch_length, batch_length, ...)
    # post.items[:, :, j] == post.items[:, j]

    steps = jnp.arange(batch_length)
    start = jnp.tile(steps, [batch_size, batch_length, 1]).transpose([0, 2, 1])
    distance = jnp.tile(steps, [batch_size, batch_length, 1])
    mask = (start + distance < batch_length)
    mask = jnp.logical_and(mask , (distance == d - 1)) if d else jnp.logical_and(mask , (distance < max_d))

    is_firsts = repeat_axis1(is_first)
    is_firsts = merge(is_firsts)
    rolls = jnp.tile(jnp.arange(batch_length, 0, -1), batch_size)
    is_firsts = indep_roll(is_firsts, rolls, axis=1)
    initials = merge(1 - is_first)
    new_mask = is_firsts.cumsum(axis=1) + initials.reshape((-1, 1))
    new_mask = unmerge(new_mask) <= 1

    mask = jnp.logical_and(mask, new_mask)

    return post, prior, mask
...
class ActorWithExtendedState(nj.Module):

  def __init__(
      self, shape, layers, units, inputs=['tensor'], dims=None,
      symlog_inputs=False, delayed_actor=None, **kw):
    ...
    self.delayed_actor = delayed_actor
    assert self.delayed_actor in ['memoryless', 'extended_state_rnn_rnn', 'extended_state_mlp']

  def _memoryless(self, inputs):
    # Copied from MLP
  ...
  def _extended_rnn(self, inputs):
    # TODO: Manipulate input for policy...
    # TODO: Edit RNN module.
    # Check for different entry point, different delays
    # Check the shape.. (batch, num_delay, action)
    # train: (0,)
    # train: (num_delay, batch, action_dim)
    # train: (t, num_delay, batch, action_dim)
    # inference: (num_delay, 1, action_dim)
    actions = inputs.pop("action_next")
    feat = self._inputs(inputs)
    if self._symlog_inputs:
      feat = jaxutils.symlog(feat)
    x = jaxutils.cast_to_compute(feat)
    ...
    for i in range(self._layers-1):
      x = self.get(f'h{i}', Linear, self._units, **self._dense)(x)
    # TODO: RNN
    x = self.get(f'h{self._layers-1}', Linear, self._units, **self._dense)(x)
    x = x.reshape(feat.shape[:-1] + (x.shape[-1],))
    if self._shape is None:
      return x
    elif isinstance(self._shape, tuple):
  ...
  def _extended_mlp(self, inputs):
    ...
    if len(actions.shape) == 3:
      actions = actions.transpose([1, 2, 0])
      actions = actions.reshape(actions.shape[0], -1)
    elif len(actions.shape) == 4:
      actions = actions.transpose([0, 2, 3, 1])
      actions = actions.reshape(actions.shape[0], actions.shape[1], -1)
    else:
      raise NotImplementedError
    feat = self._inputs(inputs)
    feat = jnp.concatenate([feat, actions], -1)  # Concat
    if self._symlog_inputs:
      feat = jaxutils.symlog(feat)
    x = jaxutils.cast_to_compute(feat)
    x = x.reshape([-1, x.shape[-1]])
  ...
  def __call__(self, inputs):
    """_summary_
    Args:
        inputs (_type_): latest delayed observation (s_t-d)
        actions (_type_): action sequences (a_t-d, ..., a_t-1)
    Raises:
        ValueError: _description_
    Returns:
        _type_: a_t
    """
    if self.delayed_actor == 'memoryless':
      return self._memoryless(inputs)
    elif self.delayed_actor == 'extended_state_rnn':
      return self._extended_rnn(inputs)
    elif self.delayed_actor == 'extended_state_mlp':
      return self._extended_mlp(inputs)
    else:
      raise NotImplementedError
